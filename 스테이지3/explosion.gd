...
func _ready():
	# Play the animation. Configuration will be handled by the projectile that creates it.
...
func set_radius_and_apply_effects(new_radius: float):
	...
	if not damage_area:
		printerr("Explosion (Stage 3): Could not find DamageArea node!")
		return
		
	var damage_shape: CollisionShape2D = damage_area.get_node_or_null("CollisionShape2D")
	if not damage_shape:
		printerr("Explosion (Stage 3): Could not find CollisionShape2D node!")
		return
