...
func _ready():
	# 물리 및 일반 프로세스를 비활성화하여 충돌 가능성을 원천 차단
	set_process(false)
	set_physics_process(false)
	
	visual_node = $Sprite2D # Get the Sprite2D node
	# 텍스처를 미리 로드해두지만, set_radius에서도 안전장치를 둠
	if not visual_node.texture:
		visual_node.texture = load("res://스테이지3/Img/warning.png") # 올바른 경로로 수정
...
func set_radius(radius: float):
	...
	# 스프라이트의 하단 중앙을 (0, 0)에 맞추기 위해 Y 위치만 조정
