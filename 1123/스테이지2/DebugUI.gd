...
func _ready():
	...
	if is_instance_valid(toggle_homing_button):
		...
		pass # printerr("DebugUI Error: 'ToggleHomingButton' 노드를 찾을 수 없습니다.")
	...
	if is_instance_valid(spawn_wall_button):
		...
		pass # printerr("DebugUI Error: 'SpawnWallButton' 노드를 찾을 수 없습니다.")
	...
	if is_instance_valid(damage_heaters_button):
		...
		pass # printerr("DebugUI Error: 'DamageHeatersButton' 노드를 찾을 수 없습니다.")

	var reset_button = get_node_or_null("ResetButton")
	if is_instance_valid(reset_button):
		reset_button.pressed.connect(_on_reset_button_pressed)
	else:
		pass # printerr("DebugUI Error: 'ResetButton' 노드를 찾을 수 없습니다.")
