...
이 문서는 2025년 12월 1일 기준으로 스테이지 2에 적용된 주요 변경 사항들을 요약합니다.
...
*   **부품 시스템 연동:** 이제 플레이어는 게임 시작 시 `PlayerData`에 저장된 부품 정보를 불러와 장착합니다.
...
*   **부품 시스템 UI 추가:** 부품을 획득하고 선택/장착할 수 있는 새로운 UI 화면들이 추가되었습니다.
...
*   부품 선택 기능과 관련된 다수의 버그 수정.

 ### 8. 옵션 및 저장 기능 추가
*   **옵션 화면:** 사운드(마스터, 음악, 효과음) 및 키보드 설정을 변경할 수 있는 옵션 화면이 추가되었습니다.
*   **키 설정 저장:** 플레이어가 변경한 키보드 설정이 `user://keybinds.cfg` 파일에 영구적으로 저장되어 게임 재시작 시에도 유지됩니다.
*   **플레이어 데이터 저장:** 플레이어가 소유하고 장착한 파츠 정보가 `user://parts_data.cfg` 파일에 영구적으로 저장되어 게임 재시작 시에도 유지됩니다.
*   **게임 초기화:** '게임 초기화' 버튼을 통해 모든 사용자 설정(키 설정, 파츠 정보)을 기본값으로 되돌리고 게임 재시작하는 기능이 추가되었습니다.
...
*   **게이지 변화:**
	*   **증가:** "얼음" 타일 위에서 초당 7, "녹은 얼음" 타일 위에서 초당 2씩 증가합니다.
...
플레이어가 부품을 영구적으로 소유하고, 원하는 부품을 선택하여 장착할 수 있는 시스템이 도입되었습니다.

#### 1.4.1. 부품 데이터 관리 (`PlayerData.gd`)
*   **역할:** Autoload 싱글톤으로, 플레이어의 데이터를 게임 세션 전반에 걸쳐 유지합니다.
*   **주요 변수:**
	*   `owned_part_paths: Array[String]`: 플레이어가 획득한 모든 부품의 리소스 경로를 저장하는 배열입니다.
	*   `equipped_parts: Array[Part]`: 플레이어가 현재 장착한 부품의 리소스 객체를 저장하는 배열입니다.
*   **주요 함수:**
	*   `add_owned_part(part_path)`: 새로운 부품을 `owned_part_paths`에 추가합니다. (중복 제외)
	*   `equip_part(new_part, slot_index)`: 특정 슬롯에 부품을 장착/해제합니다.
	*   `reset()`: `equipped_parts`와 `owned_part_paths`를 초기 상태로 초기화합니다.
*   **데이터 영속성:** `save_parts_data()` 및 `load_parts_data()` 함수가 추가되었습니다. 게임 시작 시 `user://parts_data.cfg` 파일에서 데이터를 불러오며, 부품 획득 또는 장착 시 자동으로 파일에 저장하여 데이터가 영구적으로 유지됩니다. (상세 내용은 8.3. 참조)

#### 1.4.2. 부품 선택 화면 (`PartSelectionScreen.tscn`)
*   **기능:** 플레이어는 이 화면에서 `PlayerData.owned_part_paths`에 등록된 자신이 소유한 모든 부품을 확인하고, 원하는 부품을 슬롯에 장착할 수 있습니다.
*   **데이터 연동:** 여기서 변경된 장착 정보는 `PlayerData.equipped_parts`에 즉시 저장됩니다.

#### 1.4.3. 부품 획득 (`stage2reward.tscn`)
*   **역할:** 스테이지 클리어 시 보상으로 특정 부품을 부여하는 화면입니다.
*   **동작:** '획득하기' 버튼을 누르면, 해당 부품의 리소스 경로가 `PlayerData.add_owned_part()` 함수를 통해 플레이어의 영구 소유 목록에 추가됩니다.
...
*   **부품 장착:** 게임 시작 시, `_ready()` 함수가 전역 `PlayerData` 싱글톤에서 `equipped_parts` 정보를 로드하여 자신에게 적용합니다. 이를 통해 모든 스테이지에서 플레이어가 선택한 부품 구성을 사용합니다.
...
*   **기능:** `_process` 함수 내에서 "player", "heaters" 그룹의 노드를 찾아, 그들의 `health_updated` 및 `freeze_gauge_changed` 시그널에 연결합니다. 시그널을 수신하면 `PlayerHealthLabel`, `FreezeGaugeLabel`의 텍스트를 업데이트합니다. **보스의 체력 바(`BossHealthBar`)는 `스테이지2.gd` 스크립트에 의해 관리되며, 보스의 `health_updated` 시그널을 직접 수신하여 체력 상태를 업데이트합니다.** **각 보스 온열장치의 체력은 화면 좌측 상단에 표시됩니다.**
...
### 5.2. 부품 선택 및 획득 화면
*   **부품 선택 (`PartSelectionScreen.tscn`):** 스테이지 선택 화면에서 진입할 수 있으며, 플레이어가 소유한 부품을 관리하고 장착할 수 있습니다.
*   **부품 획득 (`stage2reward.tscn`):** 스테이지 클리어 후 특정 부품을 획득했음을 알려주는 화면입니다. '획득하기' 버튼을 누르면 부품이 영구적으로 소유 목록에 추가되고 스테이지 선택 화면으로 돌아갑니다.
...
-----------------------------------------------------
8. 설정 및 데이터 영속성 (Settings & Data Persistence)
-----------------------------------------------------

게임의 편의성과 플레이 경험 유지를 위해 설정 화면 및 데이터 저장/불러오기 시스템이 추가되었습니다.

### 8.1. 옵션 화면 시스템 (`OptionScreen.tscn`)
*   **기능:** 게임의 다양한 설정을 변경할 수 있는 중앙 화면입니다. 스테이지 선택 화면에서 '옵션' 버튼을 통해 접근할 수 있습니다.
*   **사운드 설정:** 마스터, 배경음악(Music), 효과음(SFX) 볼륨을 개별적으로 조절하는 슬라이더를 제공합니다. 이 슬라이더는 `AudioServer`의 해당 오디오 버스 볼륨을 직접 제어합니다.
*   **키보드 설정 접근:** '키보드 설정' 버튼을 통해 아래의 키보드 설정 화면으로 이동할 수 있습니다.

### 8.2. 키보드 설정 시스템 (`KeybindScreen.tscn`)
*   **기능:** 플레이어가 게임 내 주요 액션(`move_left`, `fire` 등)에 할당된 키를 직접 변경할 수 있습니다.
*   **로직:**
	*   각 액션 당 하나의 키를 할당할 수 있도록 UI가 구성되어 있습니다.
	*   'Press a key...' 상태에서 새로운 키를 누르면 `InputMap`이 업데이트되어 즉시 적용됩니다.
*   **데이터 저장:** 변경된 키 설정은 `SettingsManager`에 의해 `user://keybinds.cfg` 파일에 자동으로 저장됩니다. (아래 8.3. 참조)

### 8.3. 설정 및 데이터 관리자 (`SettingsManager.gd`, `PlayerData.gd`)
*   **역할:** `SettingsManager`와 `PlayerData`는 자동 로드(Autoload) 싱글톤으로 등록되어, 게임 전반에 걸쳐 설정과 플레이어 데이터를 관리하고 영속적으로 저장/불러오기합니다.
*   **키 설정 저장/불러오기 (`SettingsManager.gd`):**
	*   **저장:** 키보드 설정 화면에서 'Back' 버튼을 누를 때, 현재 `InputMap`의 키 설정을 `user://keybinds.cfg` 파일에 저장합니다.
	*   **불러오기:** 게임 시작 시, 저장된 설정 파일을 자동으로 불러와 `InputMap`에 적용합니다. 저장된 파일이 없거나 특정 키가 비어있으면, `move_left(A)`, `move_right(D)` 등 필수적인 기본값으로 설정됩니다.
*   **플레이어 파츠 데이터 저장/불러오기 (`PlayerData.gd`):**
	*   **저장:** 플레이어가 새로운 파츠를 획득하거나(`add_owned_part`) 파츠를 장착/해제할 때(`equip_part`), 소유 및 장착 파츠 목록을 `user://parts_data.cfg` 파일에 자동으로 저장합니다.
	*   **불러오기:** 게임 시작 시, 저장된 파츠 데이터를 자동으로 불러와 플레이어의 소유/장착 상태를 복원합니다.
*   **게임 초기화 (`SettingsManager.gd`의 `reset_all_data()`):**
	*   **기능:** `user://keybinds.cfg`와 `user://parts_data.cfg` 두 파일을 삭제하고, `PlayerData` 싱글톤의 메모리 데이터를 `PlayerData.reset()`을 통해 초기화하며, `InputMap`의 키 설정을 기본값으로 재설정합니다.
	*   **재시작:** 모든 초기화가 완료된 후 `res://스테이지3/title_screen.tscn` 씬으로 전환하여 게임을 다시 시작합니다.
